...
public class DocumentServices implements InitializingBean {
    ...
    public void afterPropertiesSet() {
        ...
        save(new Documento().id(1L).nome("Contratto").descrizione("Contratto tra le parti per sottoscrizione conto corrente").data(LocalDate.now()));
        save(new Documento().id(2L).nome("Recesso").descrizione("Norme per il recesso").data(LocalDate.now()));
        save(new Documento().id(3L).nome("Appendice").descrizione("Appendice al contratto di sottoscrizione").data(LocalDate.now()));
    }
